...
def fn_target_distribution(point_list, wl, wl_min, wl_max, sd_target):
    if len(point_list) == 2:
        if point_list[0][1] == point_list[1][1]:
            mode = 1
            amplitude_1 = point_list[0][1]
        else:
            mode = 2
            amplitude_1 = point_list[0][1]
            amplitude_2 = point_list[1][1]
    else:
        mode = 3
        x_pts = []
        y_pts = []
        for i in point_list:
            x_pts.append(point_list[i][0])
            y_pts.append(point_list[i][1])

    index = 0
    while index < len(sd_target):
        if mode == 1:
            sd_target[index] = amplitude_1
        elif mode == 2:
            endpoint_1 = amplitude_1
            endpoint_2 = amplitude_2
            m = (amplitude_1 - amplitude_2) / (wl_min - wl_max)
...
def fn_ga_algorithm(led_list, point_list):

    wl_min = point_list[0][0]
    wl_max = point_list[len(point_list) - 1][0]
    wl_incr = 2
    ...
    fn_target_distribution(point_list, wl, wl_min, wl_max, sd_target)
    ...
    return current_gen[0] #fn_distribution(current_gen[0]), 
